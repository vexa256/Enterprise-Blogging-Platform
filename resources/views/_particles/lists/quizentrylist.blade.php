...
@if($entry->image)
        <div class="media">
            ...
            <a id="" class="gif-icon-a"><img class=" lazyload img-responsive" style="display: block;width:100%" alt="{{ $entry->title }}" data-src="{{ url(makepreview($entry->image, null, 'entries')) }}"></a>
...
@endif
        ...
        <div class="answer"  style="margin-left:-15px;">
            <?php $sitep = $post->entries()->where('type', 'answer')->where('source', $entry->id)->get(); ?>
                    ...
                    <li>
                        <a class="" href="javascript:"  data-answer="{{ $answers->id }}" data-result="{{ $answers->video }}" >
                            <div class="answer-cover">
                                @if($entry->video!='3')
                                    <img class=" lazyload responsive-img" alt="{{ $answers->title }}" data-src="{{ url(makepreview($answers->image, null, 'answers')) }}">
...
<section class="entry results" id="quiz_result" data-popup="{{ get_buzzy_config('BuzzyQuizzesPopup') }}" data-qtype="{{ $post->ordertype }}" >
    <div class="quiz_result_area">
        ...
        <ol style="list-style: none;margin: 0;padding: 0;">
            @foreach($entriesquizresults as $keyp => $entry)
