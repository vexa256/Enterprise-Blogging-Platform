...
<div class="entry" data-type="quizquestion" data-co="{{ $uniquid }}" @if(isset($entry->id)) data-entry-id="{{ $entry->id }}" @endif>
    ...
    <div class="moredetail questionmore">
        ...
        <div class="answertypeselection">
            <a class="button @if(isset($entry->video)) {{ $entry->video=='1' ?  'button-orange' : 'button-white'}} @else button-orange @endif clickanswertype" data-style="thdefault" data-query="1" @if(isset($entry->video)) {{  $entry->video=='1' ? 'data-curtype=on' : '' }}@else data-curtype="on" @endif><i class="fa fa-th"></i></a>
            ...
            <a class="button @if(isset($entry->video)) {{ $entry->video=='3' ?  'button-orange' : 'button-white'}} @else button-white @endif clickanswertype" data-style="thlist" data-query="3" @if(isset($entry->video)) {{ $entry->video=='3' ? 'data-curtype=on' : '' }} @endif><i class="fa fa-th-list "></i></a>
        </div>
        <div class="clearfix"></div>
        <form action="{{ $uniquid }}" onsubmit="return false">
        <div id="answer{{ $uniquid }}" class="answers @if(isset($entry->video)) @if($entry->video == '1')thdefault @else {{  $entry->video == "2" ?  'thlarge' : 'thlist' }}@endif @endif"  style="border-radius:0;border:0;padding:0;margin:0;margin-left:-15px;background: transparent;">
            @if(empty($entry->video))
                ...
                @foreach($post->entries()->where('type', 'answer')->where('source', $entry->id)->orderBy('order')->get() as $keya => $answers)

                    @include('_forms._buzzyquiz.__addanswerform', ['answer' =>  $answers])
        ...
        <div class="clearfix"></div>
        <a class="button button-white button-full submit-button entry_fetch answerbutton" data-method="Get" data-target="answer{{ $uniquid }}" data-puttype="append" data-type="answerform" href="{{ action('FormController@addnewform') }}?addnew=answer{{ Request::query('qtype')=='trivia' ? '&qtype=trivia' : '' }}"><i class="fa fa-plus-circle"></i>{{ trans('addpost.add', ['type' => trans('buzzyquiz.answer')]) }}</a>
    </div>
</div>
