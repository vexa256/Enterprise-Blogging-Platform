    ...
    <ul class="sidebar-menu">
        ...
        <li class="treeview  @if(Request::segment(2)=='config') active @endif">
            ...
            <ul class="treeview-menu">
                <li class="@if(Request::query('q')=='') active @endif"><a href="{{ action('Admin\ConfigController@index') }}"><i class="fa fa-caret-right"></i> {{ trans('admin.GeneralSettings') }}</a></li>
                <li class="@if(Request::query('q')=='social') active @endif"><a href="{{ action('Admin\ConfigController@index', ['q' => 'social']) }}"><i class="fa fa-caret-right"></i> {{ trans('admin.SocialMediaSettings') }}</a></li>
                <li class="@if(Request::query('q')=='mail') active @endif"><a href="{{ action('Admin\ConfigController@index', ['q' => 'mail']) }}"><i class="fa fa-caret-right"></i> {{ trans('admin.MailSettings') }}</a></li>
                <li class="@if(Request::query('q')=='storage') active @endif"><a href="{{ action('Admin\ConfigController@index', ['q' => 'storage']) }}"><i class="fa fa-caret-right"></i> {{ trans('v3.file_storage_settings') }}</a></li>
                <li class="@if(Request::query('q')=='others') active @endif"><a href="{{ action('Admin\ConfigController@index', ['q' => 'others']) }}"><i class="fa fa-caret-right"></i> {{ trans('admin.OtherSettings') }}</a></li>
            </ul>
        </li>
        ...
        @foreach(\App\Categories::byMain()->byActive()->orderBy('order')->get() as $cat)
            <li class="treeview  @if(Request::segment(3)==$cat->name_slug) active @endif">
                <a href="/admin/cat/{{ $cat->name_slug }}">
                    {!! $cat->icon !== null ? $cat->icon : '<i class="fa fa-' . config('buzzy.post_types.' . $cat->type. '.icon') . '"></i>' !!}
                    <span>{{ $cat->name }}</span>
                    <i class="fa fa-angle-left pull-right"></i>
                </a>
                <ul class="treeview-menu">
                    <li class="@if(Request::segment(3)==$cat->name_slug && Request::query('only')=='') active @endif"><a href="/admin/cat/{{ $cat->name_slug }}"><i class="fa fa-eye"></i> {{ trans('admin.view', ['type' => $cat->name ]) }}</a></li>
                    <li class="@if(Request::query('only')=='unapprove') active @endif"><a href="/admin/cat/{{ $cat->name_slug }}/?only=unapprove"><i class="fa fa-check-circle"></i>{{ trans('admin.Unapproved', ['type' => $cat->name ]) }} <small class="label pull-right bg-aqua">{{ $napprovenews }}</small></a></li>
                    <li class="@if(Request::query('only')=='deleted') active @endif"><a href="/admin/cat/{{ $cat->name_slug }}/?only=deleted"><i class="fa fa-trash-o"></i> {{ trans('admin.Trash', ['type' => $cat->name ]) }}</a></li>
                </ul>
            </li>
        ...
        @if(get_buzzy_config('p_buzzynews') == 'on')<li><a href="javascript:"><i class="fa fa-circle-o text-aqua"></i> <span>{{ trans('index.news') }}</span><small class="label pull-right bg-aqua">{{ $napprovenews }}</small></a></li> @endif
        @if(get_buzzy_config('p_buzzylists') == 'on')<li><a href="javascript:"><i class="fa fa-circle-o text-green"></i> <span>{{ trans('index.list') }}</span><small class="label pull-right bg-green">{{ $napprovelists }}</small></a></li> @endif
        @if(get_buzzy_config('p_buzzyquizzes') == 'on')<li><a href="javascript:"><i class="fa fa-circle-o text-purple"></i> <span>{{ trans('index.quiz') }}</span><small class="label pull-right bg-purple">{{ $unapprovequizzes }}</small></a></li> @endif
        @if(get_buzzy_config('p_buzzypolls') == 'on')<li><a href="javascript:"><i class="fa fa-circle-o text-yellow"></i> <span>{{ trans('index.poll') }}</span><small class="label pull-right bg-yellow">{{ $napprovepolls }}</small></a></li> @endif
        @if(get_buzzy_config('p_buzzyvideos') == 'on')<li><a href="javascript:"><i class="fa fa-circle-o text-red"></i> <span>{{ trans('index.video') }}</span><small class="label pull-right bg-red">{{ $napprovevideos }}</small></a></li> @endif
    </ul>
</section>