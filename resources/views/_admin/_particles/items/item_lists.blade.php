<li class="item" style="    max-height: 70px; min-height: 70px;">
    ...
    <div class="product-info">
        ...
        <span class="product-description" style="color:#ccc">
         <i class="fa fa-user" style="font-size:11px"></i> @if( $item->user)<a href="/profile/{{ $item->user->username_slug }}" target="_blank" style="color:#ccc">{{ $item->user->username }}</a>@endif <i class="fa fa-clock-o" style="margin-left:7px;font-size:11px"></i> {{ $item->created_at->diffForHumans() }}
        </span>
    </div>
</li><!-- /.item -->