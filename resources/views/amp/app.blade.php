...
<head>
    ...
    <style amp-custom>
        body.body {
            font-family: {{    get_buzzy_config('T_1_sitefontfamily', 'Arial') }};
            background: #e8e8e8;
            font-weight: 400;
            color: #363636;
            line-height: 1.44;
        ...
        .site-header {
            background: {{  get_buzzy_config('T_1_NavbarBC', '#fff') }};
    ...
    <script type="application/ld+json">{"@context":"http:\/\/schema.org\/","@type":"organization","@id":"#organization","logo":{"@type":"ImageObject","url":"{{ asset('assets/images/logo.png') }}"},"url":"{{ url('/') }}","name":"{{ get_buzzy_config('sitename') }}","description":"{{ get_buzzy_config('sitemetadesc') }}"}</script>
    <script type="application/ld+json">{"@context":"http:\/\/schema.org\/","@type":"WebSite","name":"{{ get_buzzy_config('sitename') }}","alternateName":"{{ get_buzzy_config('sitetitle') }}","url":"{{ url('/') }}"}</script>
...
<div class="amp-wrapper">
    <header itemscope itemtype="https://schema.org/WPHeader" class="site-header">
        <a href="{{ url('amp') }}" class="branding image-logo ">
            <amp-img src="{{ asset('assets/images/logo.png') }}" width="110" height="55" alt="{{  get_buzzy_config('sitename') }}" class=" amp-image-tag" sizes="(min-width: 110px) 110px, 100vw"></amp-img>
        </a>
    </header>
    <div class="wrap">
    ...
    <footer class="amp-footer ">
        <div class="amp-copyright">
                ...
                <ul class="social-list clearfix">
                    ...
                    @if(get_buzzy_config('facebookpage'))
                        <li class="social-item facebook">
                            <a href="{!!  get_buzzy_config('facebookpage') !!}" title="{{ trans('index.likeonface') }}" target="_blank">
                    ...
                    @if(get_buzzy_config('twitterpage'))
                            <li class="social-item twitter">
                                <a href="{!!  get_buzzy_config('twitterpage') !!}" title="{{ trans('index.followontwitter') }}" target="_blank">
                    ...
                    @if(get_buzzy_config('googlepage'))
                            <li class="social-item google_plus">
                                <a href="{!!  get_buzzy_config('googlepage') !!}" title="{{ trans('index.followongoogle') }}" target="_blank">
                    ...
                    @if(get_buzzy_config('instagrampage'))
                            <li class="social-item instagram">
                                <a href="{!!  get_buzzy_config('instagrampage') !!}" title="{{ trans('index.followoninstagram') }}" target="_blank">
            ...
            @if(get_buzzy_config('ampgoogleanalytics'))
            <amp-analytics type="googleanalytics"><script type="application/json">{"vars":{"account":"{{ get_buzzy_config('ampgoogleanalytics') }}"},"triggers":{"trackPageview":{"on":"visible","request":"pageview"}}}</script></amp-analytics>
            @endif
    </footer>
</div>
</body>
