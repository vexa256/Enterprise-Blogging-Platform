        ...
        <ol class="sidebar-trend">
            @foreach($lastTrendingVideos as $item)
                <li class="sidebar-trend__item">
                    <a class="sidebar-trend__item__link" href="{{ generate_post_url($item) }}" title="{{ $item->title }}" >
                        <figure class="sidebar-trend__item__body">
                            <img class="sidebar-trend__item__image lazyloadload" data-src="{{ makepreview($item->thumb, 's', 'posts') }}" alt="{{ $item->title }}" width="300" height="169">
