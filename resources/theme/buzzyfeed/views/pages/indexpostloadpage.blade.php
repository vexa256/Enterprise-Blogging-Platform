@foreach($lastFeatures as $k => $item)
            ...
            <div  class="content-timeline__link clearfix">
                <div class="content-timeline__media">
                    <figure class="content-timeline__media__image">
                        <a class="clearfix" style="display:block" href="{{ generate_post_url($item) }}"   title="{!! $item->title  !!}">
                        <img data-src="{{ makepreview($item->thumb, 's', 'posts') }}" class=" lazyload" alt="{!! $item->title  !!}" width="262" height="147" style="display: inline;">
                        </a>
                    </figure>
                </div>
                <div class="content-timeline__detail">
                    <div class="content-timeline__detail__container">
                        <div class="content-timeline__detail__social-media hide-phone" style="position: absolute;right:15px;top:10px">
                            ...
                            <div class="share-dropdown share-dropdown--{{ $item->id  }}  dropdown-container">
                                <ul>
                                    <li class="dropdown-container__item ripple buzz-share-button has-ripple" data-share-type="facebook" data-type="news" data-id="{{ $item->id  }}" data-post-url="/shared" data-title="{!! $item->title  !!}" data-sef="{{ generate_post_url($item) }}">
                                        <span class="share-dropdown__icon share-dropdown__icon--facebook"></span>
                                        <span class="share-dropdown__title">Facebook</span>
                                    </li>
                                    <li class="dropdown-container__item ripple buzz-share-button has-ripple" data-share-type="twitter" data-type="news" data-id="{{ $item->id  }}" data-post-url="/shared" data-title="{!! $item->title  !!}" data-sef="{{ generate_post_url($item) }}">
                                        <span class="share-dropdown__icon share-dropdown__icon--twitter"></span>
                                        <span class="share-dropdown__title">Twitter</span>
                                    </li>
                                    <li class="dropdown-container__item ripple buzz-share-button hide-phone has-ripple" data-share-type="gplus" data-type="news" data-id="{{ $item->id  }}" data-post-url="/shared" data-title="{!! $item->title  !!}" data-sef="{{ generate_post_url($item) }}">
                                        <span class="share-dropdown__icon share-dropdown__icon--google"></span>
                                        <span class="share-dropdown__title">Google +</span>
                                    </li>
                                    <li class="dropdown-container__item ripple buzz-share-button visible-phone has-ripple" data-share-type="whatsapp" data-type="news" data-id="{{ $item->id  }}" data-post-url="/shared" data-title="{!! $item->title  !!}" data-sef="{{ generate_post_url($item) }}">
                                        <span class="share-dropdown__icon share-dropdown__icon--whatsapp"></span>
                                        <span class="share-dropdown__title">Whatsapp</span>
                                    </li>
                                    <li class="dropdown-container__item ripple buzz-share-button has-ripple" data-share-type="mail" data-type="news" data-id="{{ $item->id  }}" data-post-url="/shared" data-title="{!! $item->title  !!}" data-sef="{{ generate_post_url($item) }}">
                        ...
                        <div class="content-timeline__detail--top hide-mobile">
                            ...
                            <?php $postatpe=\App\Categories::where("type", $item->type)->first();
                                  $postacatpe=getfirstcat($item->categories);
                                ?>
                                {!! isset($postatpe) ? '<a href="'.action('PagesController@showCategory', ['id' => $postatpe->name_slug ]) .'"> '.$postatpe->name. '</a>' : '' !!} {!! isset($item->categories) && isset($postacatpe) ? '/ <a href="'. action('PagesController@showCategory', ['id' => $postacatpe->name_slug ]).'" class="seca"> '.$postacatpe->name. '</a>' : '' !!}
                            </p>
                        </div>

                        <a href="{{ generate_post_url($item) }}"   title="{!! $item->title  !!}">
    ...
    @if(count($lastpoll)>0 and $k==4  and get_buzzy_config('p_buzzyvideos') == 'on')
    ...
    @if(count($lastpoll)>0 and $k==9  and get_buzzy_config('p_buzzypolls') == 'on')
